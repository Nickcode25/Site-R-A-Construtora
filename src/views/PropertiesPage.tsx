  ...
  return (
    <main className="inner-page">
      ...
      <section className="list-section section-light"><div className="site-container">
        ...
        <div className="results-bar"><div><strong>{filtered.length}</strong> {filtered.length === 1 ? "apartamento encontrado" : "apartamentos encontrados"}</div></div>
        {loading ? <PageLoader /> : error ? <div className="empty-state"><span>—</span><h2>Não foi possível carregar os apartamentos</h2><p>Tente novamente em alguns instantes.</p></div> : filtered.length ? <><div className="property-grid">{filtered.slice(0, visible).map((property) => <PropertyCard key={property.id} property={property} />)}</div>{visible < filtered.length && <div className="center-action"><button className="button button--dark" onClick={() => setVisible((count) => count + perPage)}>Carregar mais apartamentos</button></div>}</> : <div className="empty-state"><span>—</span><h2>{hasFilters ? "Nenhum apartamento encontrado" : "Nenhum apartamento cadastrado"}</h2><p>{hasFilters ? "Ajuste os filtros para conhecer outras opções." : "Os apartamentos cadastrados pelo painel administrativo aparecerão aqui."}</p>{hasFilters && <button onClick={clearFilters} className="button button--dark">Limpar filtros</button>}</div>}
      </div></section>
    </main>
  );
}
